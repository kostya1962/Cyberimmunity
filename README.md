...
**СОДЕРЖАНИЕ РАБОТЫ**
- [Введение](#Введение)
    - [Описание книжного интернет-магазина  "Я люблю читать!"](#Описание-книжного-интернет-магазина-"Я-люблю-читать!")
    - [Постановка задачи](#Постановка-задачи)
    - [Цели и Предположения Безопасности (ЦПБ)](#Цели-и-Предположения-Безопасности (ЦПБ))
    - [Защищаемые ценности](#Защищаемые-ценности)
- [Архитектура интернет-магазина](#Архитектура_интернет-магазина)
  ...
  - [Описание сценариев, при которых ЦБ нарушаются](#Описание-сценариев-(последовательности-выполнения-операций),-при-которых-ЦБ-нарушаются)
- [Кибериммунизация электронного магазина](#Кибериммунизация-электронного-магазина)
  - [Описание позитивных сценариев работы](#Описание-позитивных-сценариев-работы)
  - [Рекомендации по повышению безопасности web-сервиса (выполнение целей безопасности)](#Рекомендации-по-повышению-безопасности-web-сервиса (выполнение целей безопасности))
  - [Добавление необходимых модулей безопасностей](#Добавление-необходимых-модулей-безопасностей)
  - [Схема взаимодействия компонентов интернет-магазина](#Схема_взаимодействия_компонентов_интернет-магазина (после кибериммунизации))
- [Заключение](#Заключение)
- [Список использованных информационных источников](#Список-использованных-информационных-источников)
...
| Угроза  | Описание  |
|----|------|
| SQL-инъекции | SQL-инъекции возникают, когда злоумышленник может вставить вредоносный SQL-код в поля ввода, например, в форму поиска товаров или аутентификации |
...
Функциональность: Пользовательский интерфейс для просмотра книг, добавления их в корзину, просмотр личного кабинета, истории заказов, управления профилем, платных подписок, поиска книг и рекомендаций. Включает обработку корзины, оплату (интегрированную с платежными системами: банковская карта, сбп), регистрацию/авторизацию с использованием JWT и двухфакторной аутентификации (2FA), обратную связь. Обеспечивается защищенная передача данных с использованием HTTPS (TLS/SSL).

**Серверная часть (Backend):**

Технологии: Node.js с Express.js, реляционная база данных (PostgreSQL, MySQL) 

Функциональность: Обработка заказов (создание, изменение, удаление), управление пользователями (регистрация, аутентификация, профили), управление книжным каталогом (добавление, редактирование, удаление, фильтрование), интеграция с платежными системами, обработка платежей, обработка отзывов и обращений. 

**База данных (Данные):**

Технологии: PostgreSQL.

Функциональность: Хранение данных о книгах, пользователях, заказах, платежах, транзакциях. Поддержка транзакций для целостности данных, индексы для быстроты запросов, разделение данных для разных ролей (шифрование паролей) и регулярные резервные копии.

**Платежная система (МИР, Visa):**

Технологии: API платежных систем (SberBusinessAPI).

Функциональность: Обработка платежей, проверка платежей, обработка ошибок, безопасность платежной информации (HTTPS).
...
Эта работа посвящена повышению безопасности интернет-магазина “Я люблю читать!”. Мы выявили ключевые угрозы, связанные с веб-сервисом и внедрили некоторые меры по их уменьшению.

В ходе этой работы мы определили основные цели безопасности, которая включает защиту данных пользователей, предотвращает мошенничество, обеспечивает целостность и доступность веб-ресурсов. Мы проанализировали слабые места и возможные угрозы и определили сценарии, где эти угрозы могли бы реализоваться.

Внедрение рекомендаций по кибериммунизации позволило значительно повысить безопасность интернет-магазина, укрепить доверие пользователей и, способствовать росту интернет-магазина. Поддерживая высокий уровень безопасности, наш интернет-магазин гарантирует нашим клиентам простые, надёжные и комфортные условия для совершения покупок. 
